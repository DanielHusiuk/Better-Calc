...
class AreaViewController: UIViewController {
    ...
    var selectedUnits: [UIButton: UnitArea] = [:]
    let converterId: Int64 = 2
    ...
    func updatePreferences() {
        ...
        if selectedUnits[FromButtonOutlet] == nil {
            selectedUnits[FromButtonOutlet] = .squareMeters
        }
        if selectedUnits[ToButtonOutlet] == nil {
            selectedUnits[ToButtonOutlet] = .squareYards
        }
    ...
    @IBAction func HistoryButtonAction(_ sender: UIBarButtonItem) {
        saveCoreData()
        performSegue(withIdentifier: "AreaHistory", sender: self)
    }
    ...
    func UnitMenu(in button: UIButton) {
        ...
        let actions = UnitsModel().areaDictionary.keys
            .sorted{ ($0.description) < ($1.description) }
            .enumerated()
            .map { index, option in
                let description = UnitsModel().areaDictionary[option]
