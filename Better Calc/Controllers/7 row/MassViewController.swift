...
class MassViewController: UIViewController {
    ...
    var selectedUnits: [UIButton: UnitMass] = [:]
    let converterId: Int64 = 7
    ...
    func updatePreferences() {
        ...
        if selectedUnits[FromButtonOutlet] == nil {
            selectedUnits[FromButtonOutlet] = .kilograms
        }
        if selectedUnits[ToButtonOutlet] == nil {
            selectedUnits[ToButtonOutlet] = .pounds
        }
    ...
    @IBAction func HistoryButtonAction(_ sender: UIBarButtonItem) {
        saveCoreData()
        performSegue(withIdentifier: "MassHistory", sender: self)
    }
    ...
    func UnitMenu(in button: UIButton) {
        ...
        let actions = UnitsModel().massDictionary.keys
            .sorted{ ($0.description) < ($1.description) }
            .enumerated()
            .map { index, option in
                let description = UnitsModel().massDictionary[option]
