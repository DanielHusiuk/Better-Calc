...
class TimeViewController: UIViewController {
    ...
    func updatePreferences() {
        ...
        if selectedUnits[FromButtonOutlet] == nil {
            selectedUnits[FromButtonOutlet] = .weeks
        }
        if selectedUnits[ToButtonOutlet] == nil {
            selectedUnits[ToButtonOutlet] = .minutes
        }
    ...
    @IBAction func HistoryButtonAction(_ sender: UIBarButtonItem) {
        saveCoreData()
        performSegue(withIdentifier: "TimeHistory", sender: self)
    }
    ...
    func UnitMenu(in button: UIButton) {
        ...
        let actions = UnitsModel().timeDictionary.keys
            .sorted{ ($0.description) < ($1.description) }
            .enumerated()
            .map { index, option in
                let description = UnitsModel().timeDictionary[option]
