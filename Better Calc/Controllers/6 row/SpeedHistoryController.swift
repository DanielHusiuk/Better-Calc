...
class SpeedHistoryController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func saveConverterState() {
        executeExternalFunction(SpeedViewController.self) { $0.saveViewState() }
    }
    ...
    @IBAction func unwindHome(_ segue: UIStoryboardSegue) {
        ...
        executeExternalFunction(SpeedViewController.self, action: { $0.loadViewState() })
    }
    ...
    @IBAction func deleteAll(_ sender: UIBarButtonItem) {
        ...
        deleteAlert.addAction(UIAlertAction(title: NSLocalizedString("confirm", comment: ""), style: .destructive, handler: { [weak self] (action: UIAlertAction!) in
            ...
            executeExternalFunction(SpeedViewController.self, action: { $0.HistoryButtonOutlet.isEnabled = false })
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let sortedUnits = UnitsModel().speedDictionary.keys.sorted{ ($0.description) < ($1.description) }
        
        if HistoryTableView.isEditing == true {
            return
        } else {
            if let navigationController = self.presentingViewController as? UINavigationController,
               let converterVC = navigationController.viewControllers.first(where: { $0 is SpeedViewController }) as? SpeedViewController {
