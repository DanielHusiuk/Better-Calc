...
class DataHistoryController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBAction func unwindHome(_ segue: UIStoryboardSegue) {
        ...
        executeExternalFunction(DataViewController.self, action: { $0.loadViewState() })
    }
    ...
    @IBAction func deleteAll(_ sender: UIBarButtonItem) {
        ...
        deleteAlert.addAction(UIAlertAction(title: NSLocalizedString("confirm", comment: ""), style: .destructive, handler: { [weak self] (action: UIAlertAction!) in
            ...
            executeExternalFunction(DataViewController.self, action: { $0.HistoryButtonOutlet.isEnabled = false })
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if HistoryTableView.isEditing == true {
            ...
            if let navigationController = self.presentingViewController as? UINavigationController,
               let converterVC = navigationController.viewControllers.first(where: { $0 is DataViewController }) as? DataViewController {
