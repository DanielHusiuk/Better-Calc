...
class TemperatureViewController: UIViewController {
    ...
    var selectedUnits: [UIButton: UnitTemperature] = [:]
    let converterId: Int64 = 4
    ...
    func updatePreferences() {
        ...
        if selectedUnits[FromButtonOutlet] == nil {
            selectedUnits[FromButtonOutlet] = .celsius
        }
        if selectedUnits[ToButtonOutlet] == nil {
            selectedUnits[ToButtonOutlet] = .fahrenheit
        }
    ...
    @IBAction func HistoryButtonAction(_ sender: UIBarButtonItem) {
        saveCoreData()
        performSegue(withIdentifier: "TemperatureHistory", sender: self)
    }
    ...
    func UnitMenu(in button: UIButton) {
        ...
        let actions = UnitsModel().temperatureDictionary.keys
            .sorted{ ($0.description) < ($1.description) }
            .enumerated()
            .map { index, option in
                let description = UnitsModel().temperatureDictionary[option]
