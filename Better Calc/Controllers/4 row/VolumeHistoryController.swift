...
class VolumeHistoryController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBAction func unwindHome(_ segue: UIStoryboardSegue) {
        ...
        executeExternalFunction(VolumeViewController.self, action: { $0.loadViewState() })
    }
    ...
    @IBAction func deleteAll(_ sender: UIBarButtonItem) {
        ...
        deleteAlert.addAction(UIAlertAction(title: NSLocalizedString("confirm", comment: ""), style: .destructive, handler: { [weak self] (action: UIAlertAction!) in
            ...
            executeExternalFunction(VolumeViewController.self, action: { $0.HistoryButtonOutlet.isEnabled = false })
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let sortedUnits = UnitsModel().volumeDictionary.keys.sorted{ ($0.description) < ($1.description) }
        
        if HistoryTableView.isEditing == true {
            return
        } else {
            if let navigationController = self.presentingViewController as? UINavigationController,
               let converterVC = navigationController.viewControllers.first(where: { $0 is VolumeViewController }) as? VolumeViewController {
