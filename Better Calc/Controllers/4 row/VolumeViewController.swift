...
class VolumeViewController: UIViewController {
    ...
    var selectedUnits: [UIButton: UnitVolume] = [:]
    let converterId: Int64 = 3
    ...
    func updatePreferences() {
        ...
        if selectedUnits[FromButtonOutlet] == nil {
            selectedUnits[FromButtonOutlet] = .cubicCentimeters
        }
        if selectedUnits[ToButtonOutlet] == nil {
            selectedUnits[ToButtonOutlet] = .cubicMiles
        }
    ...
    @IBAction func HistoryButtonAction(_ sender: UIBarButtonItem) {
        saveCoreData()
        performSegue(withIdentifier: "VolumeHistory", sender: self)
    }
    ...
    func UnitMenu(in button: UIButton) {
        ...
        let actions = UnitsModel().volumeDictionary.keys
            .sorted{ ($0.description) < ($1.description) }
            .enumerated()
            .map { index, option in
                let description = UnitsModel().volumeDictionary[option]
